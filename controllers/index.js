...
module.exports = {
    ...
    getListTweets: (req, res) => {
        axios({
            method: "GET",
            url: "https://api.twitter.com/2/lists/1568647817408778240/tweets?tweet.fields=attachments,created_at,entities&expansions=author_id&user.fields=profile_image_url,username,verified&max_results=10",
            headers: {
                "Authorization": process.env.BEARER_TOKEN
            }
        })
            .then(result => {
                const posts = result.data.data.map(tweet => {
                    const user = result.data.includes.users.find(user => user.id === tweet.author_id);
                    return {
                        profile_name: user.name,
                        profile_username: user.username,
                        profile_img: user.profile_image_url.replace("_normal", ""),
