...
class Post extends Component {
    ...
    render() {
        return (
            <Card className="mt-4">
                <Card.Header className="px-4 py-3 bg-transparent d-flex align-items-center">
                    <div className="me-3">
                    ...
                    <div className="d-block d-lg-flex justify-content-between align-items-lg-center text-lg-end">
                        <div className="text-secondary d-flex flex-nowrap">
                            <div className="me-3 position-relative">
                                ...
                                <Badge pill bg="primary" className={`position-absolute top-0 start-50 ${!this.state.comments ? "d-none" : ""}`} style={{ fontSize: "8px" }}>
                                    {this.state.comments}
                                </Badge>
                            </div>
                            <div className="me-3 position-relative">
                                <FontAwesomeIcon icon={faRetweet} size="lg" />
                                <Badge pill bg="primary" className={`position-absolute top-0 start-50 ${!this.state.retweets ? "d-none" : ""}`} style={{ fontSize: "8px" }}>
                                    {this.state.retweets}
                                </Badge>
                            </div>
                            <div className="me-3 position-relative">
                                <FontAwesomeIcon icon={faHeart} size="lg" />
                                <Badge pill bg="primary" className={`position-absolute top-0 start-50 ${!this.state.likes ? "d-none" : ""}`} style={{ fontSize: "8px" }}>
                                    {this.state.likes}
                                </Badge>
                            </div>
                        </div>
